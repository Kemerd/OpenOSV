...
#  the tables are exactly what this build computes - the whole set, one
#  folder per output (Rec2100_PQ, Rec2100_HLG, Rec709) and its README.txt.
#  A mismatch with the committed luts\ is reported: it means the
#  repository's copies are stale.  The package spells the folder LUTs, the
#  way the installed folder is spelled; install_plugins.ps1 finds it either
#  way (Windows paths are case-insensitive).
# ---------------------------------------------------------------------------
function New-PackageLuts {
    ...
    $made = @(Get-ChildItem -LiteralPath $target -File -Recurse -Filter '*.cube')
    if ($made.Count -eq 0) {
        throw "gen_luts.ps1 produced no .cube files in '$target'."
    }
    if (-not (Test-Path -LiteralPath (Join-Path $target 'README.txt') -PathType Leaf)) {
        throw "gen_luts.ps1 wrote no README.txt into '$target'."
    }
    # Compare every generated file, the README included, with the committed
    # copy at the same place in luts\.
    $committed = Join-Path $script:RepoRoot 'luts'
    $root = (Resolve-Path -LiteralPath $target).Path.TrimEnd('\') + '\'
    foreach ($file in @($made) + @(Get-Item -LiteralPath (Join-Path $target 'README.txt'))) {
        $relative = $file.FullName.Substring($root.Length)
        $theirs = Join-Path $committed $relative
        if (-not (Test-Path -LiteralPath $theirs -PathType Leaf)) {
            Write-Warn "luts\$relative is not committed; run scripts\gen_luts.ps1 and commit it."
            continue
        }
        $a = (Get-Sha256 $file.FullName)
        $b = (Get-Sha256 $theirs)
        if ($a -ne $b) {
            Write-Warn "The committed luts\$relative differs from what this build generates; the package ships the generated one. Run scripts\gen_luts.ps1 and commit."
        }
    }
    Write-Info "LUTs: $($made.Count) tables in $(@(Get-ChildItem -LiteralPath $target -Directory).Count) folders, and README.txt"
}
...
with the LUTs in LUTs\ (its README.txt says which one to use).
...
try {
    ...
    if ($build.OfxBundle) {
        ...
        # folders and README.txt included.
