...
`LICENSE`, `NOTICE`, `CHANGELOG.md`, `SHA256SUMS.txt`, `LUTs\` (one folder per
output and a `README.txt`), `cli\`
...
* **The CLI.** `cli\osvtool.exe --version` must run with `PATH` cut down to
  ...
  `osvtool` (`scripts\gen_luts.ps1`); if any table or the README differs
  from the committed `luts\`, the script warns: regenerate and commit them.
