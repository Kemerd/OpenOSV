...
//
// The structure mirrors the paper (Kroeger et al., ECCV 2016) and, where it
// differs, mirrors DJI's shader decomposition so their recovered constants
// map onto named parameters here rather than onto magic numbers:
//
//   buildPyramid            Gaussian half-scale pyramid of both images
//   precomputeTensors       per-patch inverse structure tensor  (their
//                           precomputeStructureTensor{Hor,Ver}Shader)
//   solvePatches            inverse search per patch             (their
//                           gradientDescentFlow / processPatchMeanNorm)
//   densify                 quality-weighted scatter to pixels   (their
//                           averageNeighborhoods)
//   smoothFlow              separable Gaussian on the field      (their
//                           smooth_optical_flow)
//   repairFlow              hole filling from neighbours         (their
//                           repair_machine_flow)
//
...
/// lenses, and it is why DJI's shader is called processPatchMeanNorm rather
/// than a plain SSD.
