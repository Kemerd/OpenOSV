...
/// produces.
///
/// They are generated by scripts/gen_luts.ps1 (osvtool lut) and committed so a
/// user can load them without building anything - but that makes them a copy
/// of the colour pipeline living outside the code, and a copy silently goes
/// stale.  Change the D-Log M curve, a transfer function or a primaries matrix
/// without re-running the generator and the repository would ship tables that
/// no longer match what the plug-in renders, which is exactly the kind of
...
TEST_CASE("The committed LUTs match the current pipeline", "[color][cube]") {
    ...
    for (const Shipped& s : kShipped) {
        ...
        const std::filesystem::path fresh = osvtest::tempDir() / (std::string("regen_") + s.name);
        const Status st = writeCube(fresh, params, options);
        INFO((st.ok() ? std::string("ok") : st.error().toString()));
        REQUIRE(st.ok());
...
/// Anchor values a user can check in their NLE.
///
/// The byte comparison above proves the files are generator output but says
/// nothing about whether that output is RIGHT.  These are the BT.2408 anchors
/// read back out of the committed tables, so a curve change that happened to
/// be regenerated consistently still has to land grey and diffuse white where
/// the standard puts them.
TEST_CASE("The committed LUTs land the BT.2408 anchors", "[color][cube]") {
    const std::filesystem::path dir = osvtest::lutsDir();

    // Grid indices 26 and 46 of 65 are codes 0.40625 and 0.71875, the nearest
    // grid points to the 18 % grey (0.400) and diffuse white (0.714) codes.
    // Reading a grid point rather than interpolating keeps this a statement
    ...
    for (const Anchor& an : kAnchors) {
        ...
        REQUIRE(lut.valid());
        REQUIRE(lut.size == 65);
        // The TITLE must name OpenOSV: it is the only provenance a .cube
        // carries once it has been copied into another LUT folder.
        REQUIRE(lut.title.find("OpenOSV") != std::string::npos);

        float grey[3];
        float white[3];
        lut.at(26, 26, 26, grey);
        lut.at(46, 46, 46, white);
        // Neutral in, neutral out: the primaries matrices must not tint grey.
        for (int ch = 1; ch < 3; ++ch) {
            REQUIRE_THAT(static_cast<double>(grey[ch]), WithinAbs(static_cast<double>(grey[0]), 1e-5));
            REQUIRE_THAT(static_cast<double>(white[ch]), WithinAbs(static_cast<double>(white[0]), 1e-5));
        }
        // 5e-4 is the six-decimal file precision plus a little float slack.
        REQUIRE_THAT(static_cast<double>(grey[0]), WithinAbs(an.grey, 5e-4));
        REQUIRE_THAT(static_cast<double>(white[0]), WithinAbs(an.white, 5e-4));
    }
}
